...
class PlayerReport(QWidget):
    ...
    def init_main_UI(self):
        try:
            ...
            # Header - Create here with proper layout context
    ...
    def _calculate_season_average(self, what_to_look_for: str):
        try:
            ...
            # Check for error
            is_error, error_html = self._check_error(sum_games_stats)
            if is_error:
                return error_html
            ...
            # Check for error
            is_error, error_html = self._check_error(all_games_stats)
            if is_error:
                return error_html
    ...
    def _compare_all_games(self):
        try:
            ...
            log_entry = create_log(
                level="INFO",
                message="_compare_all_games ran successfully",
                where="_compare_all_games",
                user_id=self.user_id,
                source_ip=self.source_ip,
                request_id=self.request_id
            )
            ...
            log_entry = create_log(
                level="ERROR",
                message="_compare_all_games failed",
                where="_compare_all_games",
                error=error,
                user_id=self.user_id,
                source_ip=self.source_ip,
    ...
    def _grading(self):
        try:
            ...
            # Check for error
            is_error, error_html = self._check_error(summed_team_stats)
            if is_error:
                return {}
            
            summed_player_season_stats = AccessData.get_season_stats(player=self.players_name, sum_total=True, look_good=False)
            # Check for error
            is_error, error_html = self._check_error(summed_player_season_stats)
            if is_error:
                return {}
            ...
            log_entry = create_log(
                    level="INFO",
                    message="_grading ran successfully",
                    where="_grading",
                    user_id=self.user_id,
                    source_ip=self.source_ip,
                    request_id=self.request_id
    ...
    def _best_worst_highlights(self, what_to_look_for: str):
        try:
            ...
            # Check for error
            is_error, error_html = self._check_error(all_games)
            if is_error:
                return None
        ...
        # {'Game_1': 5, 'Game_2': 22, 'Game_3': 10}
            ...
            log_entry = create_log(
                level="INFO",
                message="_best_worst_highlights ran successfully",
                where="_best_worst_highlights",
                user_id=self.user_id,
                source_ip=self.source_ip,
                request_id=self.request_id
            )
            ...
            log_entry = create_log(
                level="ERROR",
                message="_best_worst_highlights failed",
                where="_best_worst_highlights",
                error=error,
                user_id=self.user_id,
                source_ip=self.source_ip,
    ...
    def _game_rating(self, game_name: str):
        try:
            ...
            # Check for error
            is_error, error_html = self._check_error(game_stats)
            if is_error:
                return 0
            ...
            log_entry = create_log(
                level="INFO",
                message="_game_rating ran successfully",
                where="_game_rating",
                user_id=self.user_id,
                source_ip=self.source_ip,
                request_id=self.request_id
    ...
    def _season_game_rating(self):
        try:
            ...
            # Check for error
            is_error, error_html = self._check_error(all_game_stats)
            if is_error:
            ...
            log_entry = create_log(
                level="INFO",
                message="_season_game_rating ran successfully",
                where="_season_game_rating",
                user_id=self.user_id,
                source_ip=self.source_ip,
                request_id=self.request_id
            )
            ...
            log_entry = create_log(
                level="ERROR",
                message="_season_game_rating failed",
                where="_season_game_rating",
                error=error,
                user_id=self.user_id,
                source_ip=self.source_ip,
